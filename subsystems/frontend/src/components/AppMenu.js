...
class AppSubmenu extends Component {
    ...
    renderLinkContent(item) {
		...
		let badge = (item.badge === 0) ? '' : item.badge && <span className="menuitem-badge">{item.badge}</span>;

		return (
			<React.Fragment>
				<i className={item.icon}/>
				<span className='menu-style'>{item.label}</span>
				{/*{submenuIcon}*/}
				{badge}
			</React.Fragment>
		);
    ...
    renderLink(item, i) {
		...
		if (item.to) {
			return (
				<NavLink activeClassName="active-route" to={item.to}
